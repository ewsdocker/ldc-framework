...
function pctMapList()
{
	lmsTestDisplay "${pct_colorBlock}Map Listing${lmsclr_NoColor}"
	lmsDisplayTs ""
	...
	while [ ${pct_index} -lt ${#pc_column[*]} ]
	do
		lmsTestDisplay "${pct_colorText}${pc_column[$pct_index]} = ${pc_colstart[$pct_index]} : ${pc_collen[$pct_index]}${lmsclr_NoColor}"
		(( pct_index++ ))
	done
...
function pctMapCheck()
{
	...
	do
		...
		lmsTestDisplay "${pct_colorText}Checking for proper title length:${lmsclr_NoColor}"
		lmsDisplayTs ""
