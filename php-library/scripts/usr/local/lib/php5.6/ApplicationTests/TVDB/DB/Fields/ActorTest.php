...
/*
 * 		TVDB\DB\Fields\ActorTest is copyright � 2015. EarthWalk Software.
...
class ActorTest extends \Application\Launcher\Testing\UtilityMethods
{
	/**
	 * __construct
	 *
	...
	public function assertionTests($logger=null, $format=null)
	{
		...
		$this->a_newActorFields($table);

		$this->a_iterateFields();
		$this->a_showData((string)$this->a_actorFields, 'Actor Fields');
	...
	public function a_field($field)
	{
		...
		$assertion = '$this->a_field = $this->a_actorFields->field($this->a_field);';
	...
	public function a_fields()
	{
		...
		$assertion = '$this->a_localArray = $this->a_actorFields->fields();';
		if (! $this->assertTrue($assertion, sprintf("Asserting: %s", $assertion)))
		{
			$this->a_outputAndDie();
		}
	...
	public function a_column($column)
	{
		...
		$assertion = '$this->a_column = $this->a_actorFields->column($this->a_column);';
		if (! $this->assertTrue($assertion, sprintf("Asserting: %s", $assertion)))
		{
			$this->a_outputAndDie();
		}
	...
	public function a_columns()
	{
		...
		$assertion = '$this->a_localArray = $this->a_actorFields->columns();';
		if (! $this->assertTrue($assertion, sprintf("Asserting: %s", $assertion)))
		{
			$this->a_outputAndDie();
		}
	...
	public function a_add($field, $definition, $null, $default)
	{
		...
		$assertion = '(($this->a_data = $this->a_actorFields->add($this->a_field, $this->a_definition, $this->a_null, $this->a_default)) !== null);';
		if (! $this->assertTrue($assertion, sprintf("Asserting: %s", $assertion)))
		{
			$this->a_outputAndDie();
		}
	...
	public function a_iterateFields()
	{
		...
		foreach($this->a_actorFields as $column => $value)
		{
	 ...
	 */
	public function a_newActorFields($table)
	{
		$this->labelBlock('New Actor Fields Descriptor.', 60, '*');

		$this->a_table = $table;
   		$this->a_showData($this->a_table, 'table');

		$assertion = '$this->a_actorFields = new \Application\TVDB\DB\Fields\Actor($this->a_table);';
		...
		$this->a_showData($this->a_actorFields, 'Fields Descriptor');
	}
