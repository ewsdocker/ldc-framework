...
/*
 * 		TVDB\DB\Fields\BannerTest is copyright � 2015. EarthWalk Software.
...
class BannerTest extends \Application\Launcher\Testing\UtilityMethods
{
	/**
	 * __construct
	 *
	...
	public function assertionTests($logger=null, $format=null)
	{
		...
		$table = "Banners";
		$this->a_newBannerFields($table);

		$this->a_iterateBannerFields();
		$this->a_showData((string)$this->a_bannerFields, 'Banner Fields');

		$this->a_columns();
		$this->a_column(1);
		
		$this->a_fields();
		$this->a_field('BannerPath');

		$this->a_bannerFields = null;
	}
	...
	public function a_field($field)
	{
		...
		$assertion = '$this->a_field = $this->a_bannerFields->field($this->a_field);';
	...
	public function a_fields()
	{
		...
		$assertion = '$this->a_localArray = $this->a_bannerFields->fields();';
		if (! $this->assertTrue($assertion, sprintf("Asserting: %s", $assertion)))
		{
			$this->a_outputAndDie();
		}
	...
	public function a_column($column)
	{
		...
		$assertion = '$this->a_column = $this->a_bannerFields->column($this->a_column);';
		if (! $this->assertTrue($assertion, sprintf("Asserting: %s", $assertion)))
		{
			$this->a_outputAndDie();
		}
	...
	public function a_columns()
	{
		...
		$assertion = '$this->a_localArray = $this->a_bannerFields->columns();';
		if (! $this->assertTrue($assertion, sprintf("Asserting: %s", $assertion)))
		{
			$this->a_outputAndDie();
		}
	 ...
	 */
	public function a_add($name, $definition, $null, $default)
	{
		...
		$assertion = '(($this->a_data = $this->a_bannerFields->add($this->a_name, $this->a_definition, $this->a_null, $this->a_default)) !== null);';
		if (! $this->assertTrue($assertion, sprintf("Asserting: %s", $assertion)))
		{
			$this->a_outputAndDie();
		}
	...
	/**
	 * a_iterateBannerFields
	 *
	 * Iterate the fields in the fieldDescriptor
	 */
	public function a_iterateBannerFields()
	{
		$this->labelBlock('Iterate BannerFields START', 50, '*');

		foreach($this->a_bannerFields as $column => $value)
		{
			$this->a_showData($column, sprintf("\t" . '%s', $value));
		}

		$this->labelBlock('Iterate BannerFields END', 50, '*');
	}
	 ...
	 */
	public function a_newBannerFields($table)
	{
		$this->labelBlock('New Banner Fields Descriptor.', 60, '*');

		$this->a_table = $table;
   		$this->a_showData($this->a_table, 'table');

		$assertion = '$this->a_bannerFields = new \Application\TVDB\DB\Fields\Banners($this->a_table);';
		...
		$this->a_showData($this->a_bannerFields, 'Banner Fields Descriptor');
	}
