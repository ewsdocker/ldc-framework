...
/*
 * 		TVDB\DB\Fields\SeriesTest is copyright � 2015. EarthWalk Software.
...
class SeriesTest extends \Application\Launcher\Testing\UtilityMethods
{
	/**
	 * __construct
	 *
	...
	public function assertionTests($logger=null, $format=null)
	{
		...
		$this->a_newSeriesFields($table);

		$this->a_iterateFields();
		$this->a_showData((string)$this->a_seriesFields, 'Series Fields');
	...
	public function a_field($field)
	{
		...
		$assertion = '$this->a_field = $this->a_seriesFields->field($this->a_field);';
	...
	public function a_fields()
	{
		...
		$assertion = '$this->a_localArray = $this->a_seriesFields->fields();';
		if (! $this->assertTrue($assertion, sprintf("Asserting: %s", $assertion)))
		{
			$this->a_outputAndDie();
		}
	...
	public function a_column($column)
	{
		...
		$assertion = '$this->a_column = $this->a_seriesFields->column($this->a_column);';
		if (! $this->assertTrue($assertion, sprintf("Asserting: %s", $assertion)))
		{
			$this->a_outputAndDie();
		}
	...
	public function a_columns()
	{
		...
		$assertion = '$this->a_localArray = $this->a_seriesFields->columns();';
		if (! $this->assertTrue($assertion, sprintf("Asserting: %s", $assertion)))
		{
			$this->a_outputAndDie();
		}
	...
	public function a_add($field, $definition, $null, $default)
	{
		...
		$assertion = '(($this->a_data = $this->a_seriesFields->add($this->a_field, $this->a_definition, $this->a_null, $this->a_default)) !== null);';
		if (! $this->assertTrue($assertion, sprintf("Asserting: %s", $assertion)))
		{
			$this->a_outputAndDie();
		}
	...
	public function a_iterateFields()
	{
		...
		foreach($this->a_seriesFields as $column => $value)
		{
	 ...
	 * a_newSeriesFields
	 *
	 * Create a new Series Fields Descriptor class instance
	 * @param string $table = table name
	 */
	public function a_newSeriesFields($table)
	{
		...
		$assertion = '$this->a_seriesFields = new \Application\TVDB\DB\Fields\Series($this->a_table);';
		...
		$this->a_showData($this->a_seriesFields, 'Fields Descriptor');
	}
