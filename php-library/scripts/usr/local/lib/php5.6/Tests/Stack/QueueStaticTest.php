...
/*
 *		QueueStaticTest is copyright � 2012, 2013. EarthWalk Software.
 ...
 * QueueStaticTest
 *
 * Library\Queue static class tests.
 * @author Jay Wheeler
 * @version 1.0
 ...
 * @subpackage Stack
 */
class QueueStaticTest extends \Application\Launcher\Testing\UtilityMethods
{
	/**
	 * __construct
	 *
    ...
    public function a_tail($expected)
    {
    	...
    	$assertion = '$this->a_data = \Library\Queue::tail();';
    ...
    public function a_head($expected)
    {
    	...
    	$assertion = '$this->a_data = \Library\Queue::head();';
	...
	public function a_iteratorTest($iterate=false)
	{
    	...
    	$assertion = '$this->a_testArray = \Library\Queue::queueArray();';
    ...
    public function a_setMode($mode=\SplDoublyLinkedList::IT_MODE_KEEP)
    {
    	...
    	$this->a_mode = $mode;
       	...
       	$assertion = '(($this->a_data = \Library\Queue::mode($this->a_mode)) !== null);';
    ...
    public function a_isEmpty($state=true)
    {
    	...
    	$assertion = '\Library\Queue::isEmpty();';
    	if ($state)
    	{
    ...
    {
    	$this->labelBlock('Remove ALL.', 40, '*');

    	$assertion = '$this->a_queue = \Library\Queue::removeAll();';
		if (! $this->assertTrue($assertion, sprintf('Removing ALL - Asserting: %s', $assertion)))
		{
			$this->a_outputAndDie();
		}
		...
		$this->a_exceptionCaughtFalse();
		$this->a_localArray = array();
		$this->a_compareArray(\Library\Queue::queueArray());
    }
    ...
    public function a_removeTail($expected)
    {
    	...
    	$assertion = '$this->a_data = \Library\Queue::removeTail();';
    ...
    public function a_removeQueue($key, $expected)
    {
    	...
    	$this->a_key = $key;
    	$assertion = '$this->a_data = \Library\Queue::remove($this->a_key);';
    ...
    public function a_prependQueue($data)
    {
    	...
    	$this->a_data = $data;
    	$assertion = '\Library\Queue::prepend($this->a_data);';
    ...
    public function a_addQueue($data)
    {
    	...
    	$this->a_data = $data;
    	$assertion = '\Library\Queue::add($this->a_data);';
    ...
    public function a_queueName($expected, $name=null)
    {
    	...
    	$this->a_name = $expected;
//$this->a_data = \Library\Queue::name($this->a_name);

    	$assertion = '$this->a_data = \Library\Queue::name($this->a_name);';
    ...
    public function a_newQueue($name)
    {
    	...
    	$assertion = '$this->a_queue = \Library\Queue::classInstance($this->a_data);';
