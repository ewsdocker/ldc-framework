...
use Tests\DBO\Table\DescriptorTest as DBOTableDescriptorTest;
use Library\CliParameters;
use Library\DBO\DBOConstants;

/*
 * 		MySql\TableTest is copyright � 2012, 2015. EarthWalk Software.
...
class TableTest extends DBOTableDescriptorTest // \Application\Launcher\Testing\UtilityMethods
{
	/**
	 * __construct
	 *
	...
	public function assertionTests($logger=null, $format=null)
	{
		...
		$this->a_tableExists($this->a_tableDescriptor->name);

		$this->a_dropTable($this->a_tableDescriptor);
	...
	public function a_dropTable($tableDescriptor)
	{
		...
		$assertion = '$this->a_handle->dropTable($this->a_dropDescriptor->name);';
		if (! $this->assertExceptionTrue($assertion, sprintf("Asserting: %s", $assertion)))
		{
   			$this->a_outputAndDie();
		}
